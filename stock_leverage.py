...
INVESTMENT_PERIOD = 365 * 1
INTEREST_VALUES = data['interest'].values

# None-Leveraged last values
last_value_regular = np.float16(stats_per_period(INTEREST_VALUES, INVESTMENT_PERIOD, 1, get_last)[0])
...
fig = make_subplots(cols=1, rows=len(leverages), vertical_spacing=0.03, shared_xaxes=True)
...
for leverage, color, index in zip(leverages, colors, range(1, len(leverages))):
    last_value = np.float16(stats_per_period(INTEREST_VALUES, INVESTMENT_PERIOD, leverage, get_last)[0])
    ...
    fig.add_vline(x=-2*1.1 , annotation_text=f"<b>min :{min(diff_from_norm):.2f}</b>",
